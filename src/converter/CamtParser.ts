...
export class Camt053Parser {
  ...
  private parseGroupHeader(node: Element | null, ns: string): GroupHeader {
    ...
    return {
      ...
      creationDateTime: this.parseDateTime(
        this.getTextContent(node, "CreDtTm", ns),
      ),
      additionalInformation:
        this.getTextContent(node, "AddtlInf", ns) || undefined,
    };
  }
  ...
  private parseStatement(node: Element, ns: string): Statement {
    const stmt: Statement = {
      ...
      balances: this.getElements(node, "Bal", ns).map((bal) =>
        this.parseBalance(bal, ns),
      ),
      entries: this.getElements(node, "Ntry", ns).map((entry) =>
        this.parseEntry(entry, ns),
      ),
    };

    const elctrncSeqNb = this.getTextContent(node, "ElctrncSeqNb", ns);
    if (elctrncSeqNb)
      stmt.electronicSequenceNumber = parseInt(elctrncSeqNb, 10);
  ...
  private parseBalance(node: Element, ns: string): Balance {
    ...
    return {
      ...
      creditDebitIndicator: this.getTextContent(
        node,
        "CdtDbtInd",
        ns,
      ) as CreditDebitCode,
  ...
  private parseEntry(node: Element, ns: string): Entry {
    ...
    if (bkTxCd)
      entry.bankTransactionCode = this.parseBankTransactionCode(bkTxCd, ns);
    ...
    if (this.config.parseNestedTransactions) {
      ...
      if (ntryDtls.length > 0) {
        entry.entryDetails = ntryDtls.map((dtl) =>
          this.parseEntryDetails(dtl, ns),
        );
      }
    }
  ...
  private parseEntryDetails(node: Element, ns: string): EntryDetails {
    const details: EntryDetails = {
      amount: this.parseAmount(
        this.getElement(node, "AmtDtls/TxAmt/Amt", ns),
        ns,
      ),
      creditDebitIndicator: this.getTextContent(
        node,
    ...
    const bkTxCd = this.getElement(node, "BkTxCd", ns);
    if (bkTxCd)
      details.bankTransactionCode = this.parseBankTransactionCode(bkTxCd, ns);
    ...
    if (dbtr || dbtrAcct || cdtr || cdtrAcct) {
      ...
      if (dbtrAcct)
        details.relatedParties.debtorAccount = this.parseAccount(dbtrAcct, ns);
      if (cdtr) details.relatedParties.creditor = this.parseParty(cdtr, ns);
      if (cdtrAcct)
        details.relatedParties.creditorAccount = this.parseAccount(
          cdtrAcct,
          ns,
        );
    }
    ...
    if (rmtInf)
      details.remittanceInformation = this.parseRemittanceInformation(
        rmtInf,
        ns,
      );
    ...
    if (refs) {
      details.references = {
        endToEndIdentification:
          this.getTextContent(refs, "EndToEndId", ns) || undefined,
        mandateIdentification:
          this.getTextContent(refs, "MndtId", ns) || undefined,
      };
    }
  ...
  private parseBankTransactionCode(
    ...
    return {
      ...
      subFamily:
        this.getTextContent(node, "Domn/Fmly/SubFmlyCd", ns) || undefined,
      proprietary: this.getTextContent(node, "Prtry/Cd", ns) || undefined,
    };
  }

  private parseRemittanceInformation(
    node: Element,
    ns: string,
  ): RemittanceInformation {
  ...
  private validateBalances(doc: Camt053Document): void {
    for (const stmt of doc.statements) {
      ...
      if (!opening) {
        this.warnings.push(
          `Statement ${stmt.identification}: No opening balance found`,
        );
      }

      if (!closing) {
        this.warnings.push(
          `Statement ${stmt.identification}: No closing balance found`,
        );
      }
      ...
      if (opening && closing) {
        ...
        for (const entry of stmt.entries) {
          const amount =
            entry.creditDebitIndicator === "CRDT"
              ? entry.amount.value
              : -entry.amount.value;
          calculated += amount;
        }
        ...
        const closingValue =
          closing.creditDebitIndicator === "CRDT"
